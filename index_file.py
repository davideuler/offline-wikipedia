...
archiveWriter = csv.writer(open('archive.csv','wb'))
#check for first article in the chunk, create a folder with first alphabet of that article, move whole chunk to it.... consider it to e secondary
#create separate tables for different articles, like table with first alphabet, and numerical table, and create index for each table.
archiveWriter = csv.writer(open('archive.csv','wb'))
...
while True:		
	...
	if len(title)==0:
		continue
	#elif (title[0] == )
	archiveWriter.writerow([title.encode('utf-8'),num,beg,end])			
